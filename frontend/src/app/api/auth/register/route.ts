...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail:
          "Cannot reach the API backend. Start FastAPI (e.g. `docker compose up`) or set API_BASE_URL in `.env` — for `npm run dev` use http://127.0.0.1:8000 when the API runs on the host.",
      },
      { status: 503 },
    );
  }
